...
use api::{
    AppState, acknowledge_data_move_result, acknowledge_restore_result, analyze_docx,
    ...
    check_question_duplicate, check_question_duplicates_batch, clear_paper_recovery,
    configure_template_regions, copy_paper, create_backup, create_chapter,
    create_excel_import_template, create_initial_subject, create_subject, create_tag,
    delete_chapter, delete_document_entry_template, delete_document_question_draft, delete_paper,
    delete_papers, delete_question_draft, delete_question_type, delete_subject, delete_tag,
    delete_template, delete_word_import_draft, draw_random_questions, export_paper_docx,
    export_question_bank, get_document_question_draft, get_last_data_move_result,
    get_last_restore_result, get_managed_image, get_paper, get_paper_recovery, get_question,
    get_question_draft, get_question_stem_summaries, get_settings,
    get_template_configuration_preview, get_template_layout_preview, get_word_import_draft,
    ignore_question_duplicate, import_template, inspect_backup, inspect_excel_workbook,
    list_backups, list_document_entry_templates, list_papers, list_question_drafts,
    list_question_types, list_questions, list_templates, move_questions_to_recycle,
    open_data_directory, open_exported_file, permanently_delete_questions, pick_backup_file,
    pick_backup_save_path, pick_data_directory, pick_docx_file, pick_docx_save_path,
    pick_export_directory, pick_xlsx_file, pick_xlsx_save_path, prepare_data_move, prepare_restore,
    read_wps_clipboard_images, rename_template, restore_questions, retry_database_initialize,
    ...
    save_imported_questions, save_paper, save_paper_recovery, save_question, save_question_draft,
    save_question_type, save_question_type_order, save_questions, save_settings,
    save_subject_order, save_word_import_draft, scan_question_duplicates, schedule_data_move,
    schedule_restore, set_default_document_entry_template, set_default_template,
    store_managed_images, update_chapter, update_subject, update_tag,
};
use tauri::Manager;

async fn initialize_data_root(data_root: std::path::PathBuf, app_version: String) -> AppState {
    match restore::apply_pending_restore(&data_root).await {
        Ok(_) => AppState::initialize(data_root, app_version).await,
        Err(error) => AppState::unavailable(
            data_root,
            app_version,
...
pub fn run() {
    tauri::Builder::default()
        .manage(mini_cloud::MiniCloud::default())
        ...
        .setup(|app| {
            ...
            let state = tauri::async_runtime::block_on(async {
                ...
                let mut selected_state = if let Some(error) = pending_validation_error {
                    ...
                    initialize_data_root(startup.selected_data_root.clone(), app_version.clone())
                        .await
                };
                ...
                match data_move::finalize_pending_startup(&bootstrap_dir, &startup, selected_error)
                {
                    Ok(Some(result)) if result.outcome == "rolled_back" => {
                        ...
                        selected_state = match data_move::validate_existing_data_root(&fallback) {
                            Ok(()) => initialize_data_root(fallback, app_version.clone()).await,
                            Err(error) => AppState::unavailable(
                                fallback,
                                app_version.clone(),
                                format!(
        ...
        .invoke_handler(tauri::generate_handler![
            mini_cloud::mini_cloud_login,
            mini_cloud::mini_cloud_logout,
            mini_cloud::mini_cloud_request,
            app_initialize,
